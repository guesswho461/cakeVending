...
const checkModuleAliveInterval = 1 * 60 * 1000; //ms
const maxModuleDeadCnt = 0;
const maxMachTemp = 70; //Celsius
const bowlCntWarningLevel = 60;
const bowlCntAlarmLevel = 40;
const batterLevelWarningLevel = 14; //greater means lower
const batterLevelAlarmLevel = 16; //greater means lower
let maxFridgeTemp = 20; //Celsius
const checkGateCmdDelay = 1 * 60 * 1000; //ms
...
mqttClient.on("message", function (topic, message) {
  if (topic === "bucket/status/alive") {
    ...
    if (batterLevel >= batterLevelAlarmLevel) {
      postAlarm("out of batter");
    } else if (batterLevel >= batterLevelWarningLevel) {
